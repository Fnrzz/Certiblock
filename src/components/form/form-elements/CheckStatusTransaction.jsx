...
import { checkStatusTransaction } from "@/services/certificate/checkStatusTransaction";
import { Modal } from "@/components/ui/modal";
import { useModal } from "@/app/hooks/useModal";
import { formatDate } from "@/utils/formatDate";

const CheckStatusTransaction = () => {
  ...
  const handlerForm = async (e) => {
    ...
    try {
      ...
      const transactionHash = Object.fromEntries(formData.entries());

      const result = await checkStatusTransaction(
  ...
  return (
    <>
      ...
      <Modal isOpen={successModal.isOpen} className="max-w-[600px] p-5 lg:p-10">
        ...
        <h6 className="mb-2 text-sm text-gray-600 dark:text-gray-400">
          Certificate Onchain Hash :
        </h6>
        <p className="mb-3 text-sm text-gray-400 break-all text-left">
          {data.certificateHashOnchain !== null
            ? data.certificateHashOnchain
            : "NULL"}
        </p>
        <h6 className="mb-2 text-sm text-gray-600 dark:text-gray-400">
          Transaction Fee :
        ...
        <p className="mb-3 text-sm text-gray-400 break-all text-left">
          {data?.transactionFee} POL
        </p>
        <h6 className="mb-2 text-sm text-gray-600 dark:text-gray-400">
          Confirmed At :
        </h6>
        <p className="mb-3 text-sm text-gray-400 break-all text-left">
          {data ? formatDate(data.confirmedAt) : "N/A"}
        </p>
        <div className="flex items-center justify-center w-full gap-3 mt-7">
          <button
            type="button"
            onClick={() => handlerCloseModal("success")}
      ...
      <Modal isOpen={errorModal.isOpen} className="max-w-[600px] p-5 lg:p-10">
        <div className="text-center">
          ...
          <div className="flex items-center justify-center w-full gap-3 mt-7">
            <button
              onClick={() => handlerCloseModal("error")}
              type="button"
