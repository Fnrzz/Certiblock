...
const DropzoneVerifyComponent = () => {
  ...
  const onDrop = async (acceptedFiles) => {
    if (acceptedFiles.length > 0) {
      ...
      try {
        const result = await verifyCertificateFile(file);
    ...
    <>
      ...
      <Modal isOpen={successModal.isOpen} className="max-w-[600px] p-5 lg:p-10">
        ...
        <p className="mb-3 text-sm text-gray-400 break-all text-left">
          {verificationResult ? verificationResult.dataOnChain : ""}
        </p>
...
export default DropzoneVerifyComponent;
